...
def record_audio_mic(duration, fs=8000, symmetric_key=None):
    setup()
    recorded_data = []
...
def record_audio_headphone(duration, fs=8000, symmetric_key=None):
    setup()
    recorded_data = []
    start_time = time.time()
    while (time.time() - start_time) < duration:
...
def play_audio_mic(encrypted_data, fs=8000, symmetric_key=None):
    setup()
...
def play_audio_headphone(encrypted_data, fs=8000, symmetric_key=None):
    setup()
    data = decrypt_data(encrypted_data, symmetric_key)
...
if __name__ == "__main__":
    symmetric_key = # ... derived symmetric key ...
